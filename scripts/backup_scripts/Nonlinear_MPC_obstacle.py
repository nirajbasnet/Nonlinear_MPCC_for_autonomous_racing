...
class MPC:
    ...
    def compute_optimization_cost(self):
        ...
        for k in range(self.N):
            ...
            if_else(st_next_euler[3]>self.element_arc_lengths[-1],st_next_euler[3]-self.element_arc_lengths[-1],st_next_euler[3])
    ...
    def construct_warm_start_soln(self,initial_state):
        ...
        for k in range(1,self.N+1):
            s_next = mod((self.X0[k-1,3] + self.p_initial*self.dT),self.element_arc_lengths[-1])
    ...
    def filter_estimate(self):
        for k in range(1,self.N+1):
            self.X0[k,3]=mod(self.X0[k,3],self.element_arc_lengths[-1])
    ...
    def solve(self, initial_state):
        ...
        for k in range(self.N):  #set the reference controls and path boundary conditions to track
            ...
            self.lbg[self.n_states - 1 + (self.n_states + 2) * (k + 1), 0] = -inf
            self.ubg[self.n_states - 1 + (self.n_states + 2) * (k + 1), 0] = 0
        ...
        # obstacle parameters
        p[self.n_states + 2 * self.N:self.n_states + 2 * self.N+4]= [0.0,0.0,1.0,0.0]  #x,y,yaw,velocity
...
class MPCKinematicNode:
    def __init__(self):
        ...
        self.param = {'dT': rospy.get_param('dT', 0.25),
                      ...
                      'v_max': rospy.get_param('max_speed', 2.25),
                      ...
                      'mpc_w_cte': rospy.get_param('mpc_w_cte', 30),
                      'mpc_w_epsi': rospy.get_param('mpc_w_epsi', 50),
                      'mpc_w_s': rospy.get_param('mpc_w_s', 0),
                      'mpc_w_lag': rospy.get_param('mpc_w_lag', 1200),
                      'mpc_w_vel': rospy.get_param('mpc_w_vel', 5),
                      'mpc_w_delta': rospy.get_param('mpc_w_delta', 0),
                      'mpc_w_p': rospy.get_param('mpc_w_p', 100),
                      'mpc_w_accel': rospy.get_param('mpc_w_accel', 10),
                      'mpc_w_delta_d': rospy.get_param('mpc_w_delta_d', 700),
        ...
        self.CONTROLLER_FREQ = rospy.get_param('controller_freq', 12)
