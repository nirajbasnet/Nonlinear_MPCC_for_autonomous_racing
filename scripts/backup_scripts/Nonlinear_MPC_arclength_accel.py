...
class MPC:
    ...
    def init_system_model(self):
        ...
        states = vertcat(x, y, psi,v, s)
        controls = vertcat(a, theta, p)
        self.n_states = states.size1()
        self.n_controls = controls.size1()
        self.T_V = self.n_states + self.n_controls
        rhs = vertcat(v * cos(psi), v * sin(psi), (v / self.L) * tan(theta),a, p)  # dynamic equations of the states
        ...
        self.R = MX.zeros(3, 3)
        self.R[0, 0] = self.param['mpc_w_accel']  # use of velocity control
    ...
    def compute_optimization_cost(self):
        ...
        for k in range(self.N):
            ...
            dx, dy = self.center_lut_dx(st_next[self.n_states-1]), self.center_lut_dy(st_next[self.n_states-1])
            t_angle = atan2(dy, dx)
            ref_x, ref_y = self.center_lut_x(st_next[self.n_states-1]), self.center_lut_y(st_next[self.n_states-1])
            e_c = sin(t_angle) * (st_next[0] - ref_x) - cos(t_angle) * (st_next[1] - ref_y)
            e_l = -cos(t_angle) * (st_next[0] - ref_x) - sin(t_angle) * (st_next[1] - ref_y)
            err_vel =st_next[3]-1.5
            error = vertcat(e_c, e_l)

            self.obj = self.obj + mtimes(mtimes(error.T, self.Q), error) +err_vel*err_vel*self.param['mpc_w_vel']+ mtimes(
    ...
    def init_constraints(self):
        ...
        for k in range(self.N + 1):
            self.lbx[self.n_states * k:self.n_states * (k + 1), 0] = np.array(
                [[self.x_min, self.y_min, self.psi_min,self.v_min, self.s_min]])
            self.ubx[self.n_states * k:self.n_states * (k + 1), 0] = np.array(
                [[self.x_max, self.y_max, self.psi_max,self.v_max, self.s_max]])
        ...
        for k in range(self.N):
            self.lbx[state_count:state_count + self.n_controls, 0] = np.array(
                [[-1.5, self.theta_min, self.p_min]])  # v and theta lower bound
            self.ubx[state_count:state_count + self.n_controls, 0] = np.array(
                [[1.5, self.theta_max, self.p_max]])  # v and theta upper bound
    ...
    def get_path_constraints_points(self,prev_soln):
        ...
        for k in range(1,self.N+1):
            right_points[k-1,:] = [self.right_lut_x(prev_soln[k,4]), self.right_lut_y(prev_soln[k,4])]  # Right boundary
            left_points[k-1,:] = [self.left_lut_x(prev_soln[k,4]), self.left_lut_y(prev_soln[k,4])]  # Left boundary
    ...
    def solve(self, initial_state):
        ...
        # print self.X0
        self.filter_estimate(initial_state[4])
        initial_state[4]=self.X0[0,4]
        ...
        for k in range(self.N):  #set the reference controls and path boundary conditions to track
            ...
            theta_ref = 0
            a_ref=0
            p[self.n_states+2*self.N+4*self.N_OBST+self.n_controls*k:self.n_states+2*self.N+4*self.N_OBST+self.n_controls*(k+1)] = [a_ref,theta_ref, p_ref]
...
class MPCKinematicNode:
    def __init__(self):
        ...
        self.param = {'dT': rospy.get_param('dT', 0.2),
                      ...
                      'v_max': rospy.get_param('max_speed', 2.05), #2.25
                      'p_min': rospy.get_param('p_min', 0),
                      'p_max': rospy.get_param('p_max', 4.0),
                      ...
                      'mpc_w_cte': rospy.get_param('mpc_w_cte', 100), #50
                      'mpc_w_epsi': rospy.get_param('mpc_w_epsi', 50),
                      'mpc_w_s': rospy.get_param('mpc_w_s', 0),
                      'mpc_w_lag': rospy.get_param('mpc_w_lag', 500),  #1000
                      'mpc_w_vel': rospy.get_param('mpc_w_vel', 5),
                      'mpc_w_delta': rospy.get_param('mpc_w_delta', 40),
                      'mpc_w_p': rospy.get_param('mpc_w_p', 30),  #30
                      'mpc_w_accel': rospy.get_param('mpc_w_accel', 2),
                      'mpc_w_accel_d': rospy.get_param('mpc_w_accel_d', 1),
                      'mpc_w_delta_d': rospy.get_param('mpc_w_delta_d', 600), #600
    ...
    def controlLoopCB(self, event):
        ...
        if self.goal_received and not self.goal_reached:
            ...
            print(car_pos,near_idx,current_s,v,psi)
