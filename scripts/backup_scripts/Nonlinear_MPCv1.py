...
class MPC:
    ...
    def init_system_model(self):
        ...
        rhs = vertcat(v * cos(psi), v * sin(psi), (v / self.L) * theta)
        self.f = Function('f', [states, controls], [rhs])  # nonlinear mapping function f(x,u)
        self.U = SX.sym('U', self.n_controls, self.N)

        self.P = SX.sym('P', self.n_states + self.N * (self.n_states + self.n_controls))
        ...
        self.Q[0, 0] = 0
        self.Q[1, 1] = 10
        self.Q[2, 2] = 10  # weighing matrices (states)

        self.R = SX.zeros(2, 2)
        self.R[0, 0] = 5
        self.R[1, 1] = 1.5  # weighing matrices (controls)
    ...
    def get_psides(self, x0):
        ...
        n = len(self.coeffs) - 1
        for i in range(0, n):
            psides0 += (n - i) * self.coeffs[i] * power(x0, n - i - 1)
        angle = atan(psides0)
        return angle
    ...
    def compute_optimization_cost(self):
        ...
        for k in range(self.N):
            ...
            self.obj = self.obj + mtimes(mtimes((st - ref).T, self.Q), (st - ref)) + mtimes(
                mtimes((con - self.P[6 + self.T_V * k:8 + self.T_V * k]).T, self.R),
                (con - self.P[6 + self.T_V * k:8 + self.T_V * k]))
    ...
    def solve(self, initial_state):
        p = np.zeros((self.n_states + self.N * (self.n_states + self.n_controls), 1))
        ############
        # fill in the solver initialization part for new optimization function, constraints and rest of the things are same
        ###########
        ...
        for k in range(self.N):  # new - set the reference to track
            ...
            v_ref = 0.75
            theta_ref = 0
            p[self.n_states + self.T_V * k:2 * self.n_states + self.T_V * k, 0] = [x_ref, y_ref, psi_ref]
            p[2 * self.n_states + self.T_V * k:2 * self.n_states + self.n_controls + self.T_V * k, 0] = [v_ref,
                                                                                                         theta_ref]
...
class MPCKinematicNode:
    def __init__(self):
        ...
        self.param = {'dT': rospy.get_param('dT', 0.15),
    ...
    def publish_path_from_coeffs(self, coeffs):
        ...
        for x in x_coords:
            ...
            tempPose.pose.position.y = self.mpc.polyeval(coeffs, x)
    ...
    def controlLoopCB(self, event):
        ...
        if self.goal_received and not self.goal_reached:
            ...
            current_state= vertcat(0.0,0.0,0.0)

            self.mpc.compute_optimization_cost()
            self.mpc.init_ipopt_solver()
