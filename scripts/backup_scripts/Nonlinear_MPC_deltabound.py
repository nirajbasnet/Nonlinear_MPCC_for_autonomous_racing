...
class MPC:
    def __init__(self):
        ...
        self.d_v_min, self.d_theta_min, self.d_p_min = None,None,None
        self.d_v_max, self.d_theta_max, self.d_p_max = None,None,None
    ...
    def set_initial_params(self, param):
        ...
        self.N = param['N']
        self.L = param['L']
        self.d_v_min, self.d_theta_min, self.d_p_min = -param['d_v_bound'],-param['d_theta_bound'],-param['d_p_bound']
        self.d_v_max, self.d_theta_max, self.d_p_max = param['d_v_bound'],param['d_theta_bound'],param['d_p_bound']
    ...
    def compute_optimization_cost(self):
        ...
        for k in range(self.N):
            ...
            self.obj = self.obj + mtimes(mtimes(error.T, self.Q), error) - self.param['mpc_w_p'] * con[2] + self.param[
                'mpc_w_delta'] * con[1] * con[1]
            if k < self.N - 1:
                con_next = self.U[:, k + 1]
        ...
        for k in range(self.N - 1):
            self.g = vertcat(self.g, self.U[:, k + 1] - self.U[:, k])
    ...
    def init_constraints(self):
        ...
        self.lbg = np.zeros((self.n_states * (self.N + 1) + self.N + self.n_controls * (self.N - 1), 1))
        self.ubg = np.zeros((self.n_states * (self.N + 1) + self.N + self.n_controls * (self.N - 1), 1))

        for k in range(self.N - 1):
            self.lbg[self.n_states * (self.N + 1) + self.N + self.n_controls * k:self.n_states * (
                        self.N + 1) + self.N + self.n_controls * (k + 1)] = np.array(
                [[self.d_v_min, self.d_theta_min, self.d_p_min]])
            self.ubg[self.n_states * (self.N + 1) + self.N + self.n_controls * k:self.n_states * (
                    self.N + 1) + self.N + self.n_controls * (k + 1)] = np.array(
                [[self.d_v_max, self.d_theta_max, self.d_p_max]])
...
class MPCKinematicNode:
    def __init__(self):
        ...
        self.CENTER_TRACK_FILENAME = rospy.get_param('center_track_filpath', path_folder_name+'centerline_waypoints.csv')
        self.CENTER_DERIVATIVE_FILENAME = rospy.get_param('center_derivative_filpath',
                                                          path_folder_name+'center_spline_derivatives.csv')
        self.RIGHT_TRACK_FILENAME = rospy.get_param('right_track_filpath', path_folder_name+'right_waypoints.csv')
        self.LEFT_TRACK_FILENAME = rospy.get_param('left_track_filpath', path_folder_name+'left_waypoints.csv')
