...
class MPC:
    ...
    def init_system_model(self):
        ...
        self.R[1, 1] = 10  # use of steering actuator.  weighing matrices (controls)

        self.S = SX.zeros(2, 2)
        self.S[0, 0] = 5  #change in velocity i.e, acceleration
        self.S[1, 1] = 5  #change in steering angle. weighing matrices (change in controls)
...
class MPCKinematicNode:
    ...
    def plot_data(self):
        ...
        # plt.ylim(-0.5, 1.0)
        plt.ylabel('cross track error(cte) in metres')
        plt.show()
